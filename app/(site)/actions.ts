...
/**
 * 获取静态内容页面数据 (STATIC_CONTENT type)
 */
export async function getStaticContentPage(slug: string, locale: string) {
  const pageType = await prisma.pageType.findUnique({
  ...
  if (!pageType || pageType.type !== 'STATIC_CONTENT') return null
  ...
  return {
    ...
    contentBlocks: pageType.contentBlocks.map((block) => ({
      id: block.id,
      blockType: block.blockType,
      blockKey: block.blockKey,
      sortOrder: block.sortOrder,
      title: getTranslatedField(block.translations, locale, "title", ""),
      content: getTranslatedField(block.translations, locale, "content", ""),
      buttonText: getTranslatedField(block.translations, locale, "buttonText", ""),
      buttonUrl: getTranslatedField(block.translations, locale, "buttonUrl", ""),
    })),
  }
}
